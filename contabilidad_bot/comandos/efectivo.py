...
def efectivo(bot, update, args):
    ...
    ret = automata.process(args, GRAPH_ADD_PAYMENT, 's', ACTIONS_ADD_PAYMENT, resultado)
    ...
    if(ret):
      ...
      connection[db_name]['gastos'].insert_one(resultado)
      ret_txt = 'Cobro contabilizado: ' + resultado['descripcion'] + \
                 '\nimporte: ' + resultado['importe'] + \
                 '\ntags: ' + ' '.join(resultado['tags'])
    else:
      ret_txt = '/efectivo - Agrega un gasto en efectivo, el formato es: descripción importe tags (con # adelante), ej: almacén 134.34 #comida #gasto_corriente'
